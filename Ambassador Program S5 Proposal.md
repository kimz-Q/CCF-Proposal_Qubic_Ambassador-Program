# **CCF Proposal - Ambassador Program SO5**

## **CCF Proposal**

Request to approve the budget of the Ambassador Program Season 5 for the months November, December with approx 49bln Qubic / 1000$ per/bln.

**Wallet:** FAVIWZQYSLUQTGOQYJNQFTJNXPKDYMAOOZOHUNEOBAUXUFTEZYXPPUZCYIHE

### **Available Options:**

**Option 0:** No, I don't want

**Option 1:** Yes, approve the budget of 49bln QUBIC

The Qubic Ambassador Program is a global initiative aimed at building local communities that are passionate about Qubic. They are our boots on the ground. Qubican's (ambassadors) lead on expanding Qubic's global reach by organising local events, educating local communities, and contributing to the growth of the Qubic ecosystem by integrating more closely with the ecosystem, marketing and operation teams. At this the program has over 120 active ambassadors and over 3k members across the globe.

Cryptocurrency markets are currently experiencing downward price pressure, affecting treasury stability, project growth funding, and community morale across the broader industry.

However, for Qubic, this period represents **strategic opportunity**:

- Developer attention is shifting toward platforms with strong technical foundations rather than short-term hype.

- Competing ecosystems are investing heavily into university pipelines, technical onboarding, and global expansion.

- Qubic's deterministic computing model, unique tokenomics, and UPOW mining approach position it as a high-innovation platform in a space with limited technical differentiation.

**The focus of Season 5 is to ensure that Qubic:**

1.  Continues expanding its global footprint despite market downturns.

2.  Onboards real developers who will deploy smart contracts and applications on the network.

3.  Maintains community cohesion and momentum during low-price periods.

4.  Strengthens institutional and educational footholds (universities, incubators, dev groups).

The season covers a period of 2 months with a possible extension if the Qubic price further appreciates. We have cut our budget about 10K less than S4 as we understand PA conditions not favorable at the moment.

**Link to full proposal (incl ambassador introduction, governance, CL and workgroup requirements):**
[Google Doc - Full Proposal](https://docs.google.com/document/d/1_h-6Aa0sXAha8cbaf3CWvIK60l80IOj3TXuKGKg3zxY/edit?usp=sharing)

**Link to S5 FAQ:**
[GitHub - S5 FAQ](https://github.com/kimz-Q/CCF-Proposal.-Qubic-ambassador-program-S5/blob/fb2c2bcffc3a87850873df5e04d79113d03fa52f/FAQ_S5_Ambassador_Program.md)

**Link to Ambassador Code of Conduct:**
[GitHub - Code of Conduct](https://github.com/Qubic-Core-Services/Proposals/blob/main/Qubic%20Ambassador%20Code%20of%20Conduct%20.md)

**Link to expenses tracking:**
[Google Doc - Expense Tracking](https://docs.google.com/spreadsheets/d/1qhW6aVj_Msuf3gL9kg8sbBl7e8iui6ao_8oF2ziRAHk/edit?usp=sharing)

## **What are our Season 5 Objectives**

### **The ambassador program will focus on achieving 7 objectives in Season 5:**

- **Developer Adoptation:** We aim to accelerate developer adoption and attract targeted Devs groups by creating a structured, accessible pathway into the Qubic ecosystem.

- **Maximize visibility:** Through continued creative execution and strong program commitment, we will maintain consistent multi-language content distribution across social media channels to reach and engage diverse global audiences.

1.  Bi-Weekly Progression report on AMbassador channel.

2.  Monthly AMA to show overall program progress.

3.  communities update and latest projects through Ambassador X account.

- **Partner universities**: Signing collaborations with at least 3-4 universities to enable students to create their own project with Qubic.

- **Event Participation**:Drive the successful delivery and impact of a global event portfolio. Key initiatives include managing Hack the Future and cryptoXR, while expanding our footprint through dedicated events in Vietnam and China, and leveraging the LATAM partnership with CryptoConection.

- **Unlock investors**create a visible pathway for investors to understand, evaluate, and participate in Qubic's growth by providing a clear info pack for investors , participating in local crypto group/ Web3 Meetups.

- **Strengthen current and new communities** (USA, South Korea, and Japan) and launch new ones in important strategic locations in Asia (Singapore)and Argentina.

- **Ecosystem contribution :** Communities to strategically build working teams to build SC & Dapps on Qubic network

.

## **KPI Summary**

| **Category** | **KPI** | **Target by EoY** |
| :--- | :--- | :--- |
| Growth | Total ambassadors | 180+ active contributors |
| Content | Social media | 40M+ impressions across all communities |
| Events | Events & meetups | 3+ "large" event, 5 community event |
| Investors | Unlock investors | Ready to go info Pack for investors, 2-3 introductions |
| Ecosystem | Dev/Uni support | 3-4 Uni-campuses , 6 Dapps/Scs\ Onboard(Hackathon) 3-4 new Dev groups |
| Retention | Active ambassador retention | 80% |
| Decentralization | Self-managed chapters | 13+ |

**Season 4 Review**

### **KPI Summary**

| **Category** | **KPIs** | **Planned Targets** | **Targets Achieved** |
| :--- | :--- | :--- | :--- |
| Growth | Total ambassadors | 180+ active contributors | **Over 120 with more than 3k ambassadors across the globe** |
| Content | X/Twitter reach | 20M+ impressions | **At least +25M impressions** |
| Events | Events & meetups | 5+ "large" event, 5 community event | - **Shanghai blockchain week.**<br>- **Lablab hackathon**<br>- **TOKEN2049**<br>- **The merge**<br>- **Lagos Web3** |
| Ecosystem | BD/Uni support | 5 campuses, 5 BD introductions to companies or institutions | **1 RFP : Solana brige, Cointribune : 5 new articles, TerritorioBlockchain : 2 new articles, 4 new SC pending (Qbuild, ico portal, qusino, Qubic Crowfunding) 1 SC assist : QLOAN**<br><br>**4 campuses, 3 BD introductions to institutions, 1 Hackhaton (december), Partnership with two French schools: meeting in January (CryptoXR)** |
| Retention | Active ambassador retention | 70% | **80%** |
| Decentralization | Self-managed chapters | 15+ | **11 Chapters** |

## **Other achievements:**

- **The program grew exponentially from 70 to over 3k ambassadors across the globe.**

- **Creation of two new communities: Singapore , LATAM , regions all relevant Qubics ecosystem growth plan**

- **dev workshops were held by different communities.**

- **Regular translation of Qubic material to different languages (Russian, Vietnamese,Chinese ,Japanese) which helps onboard and market the project on those regions.**

- **Organized events across France, Spain, Vietnam and Nigeria but also supported Qubic core teams at TOKEN2049 . As a result we onboarded New Singapore CLs and in communication with new Asian Devs to be onboarded.**

- **Strong connections with universities in Vietnam has established which will lead for more deep collaboration in Season 5 ( FPT,UTH,BK).**

- **The Spanish community started to build strong relationships with universities (IE,UPM, UCM) that will result in future projects and project presentations to a wide range of university students.**

- **Initial contact with El Salvador blockchain community to promote Qubic**

- **The Nigerian community partnered with Blockchain Lautech to promote Qubic awareness in the university.**

- Chinese community expand BD with infrastructure Lead companies in China that are very interested in UPOW protocol.

- France, Nomad , Spain USA , Created Dapps & Sc that are being finalized to be deployed on the network based on Quorum decision.

- Over 25mln impressions across our X posts, across all communities.

- Ongoing Collaboration with Media partners to leverage connection to provide a media backbone with program efforts.

- Integration of The Qsletter within Marketing CRM platform for wide use and more Professional mode.

- Qubic Ambassador Program X account grew from 159k to 330k impressions.

## **S4 Challenges & learnings**

- we found is very needed that we implement the Multisig vault protocol to make sure that funds are safely controlled as one person having the only access to the community wallet can lead to transparency issues.

- The recent price decline introduced a significant challenge to maintaining ambassador motivation and operational capacity. Several communities experienced a reduction of nearly 60% in the value of their allocated funds, impacting their ability to operate effectively. This required the implementation of specific measures to ensure financial continuity and to safeguard each community's ability to deliver on its responsibilities at an optimal level.

- Some communities naturally grow and perform faster than others. The primary factor observed is the varying backgrounds, skill sets, and leadership capabilities of their Community Leads (CLs), which directly influence how effectively each community is guided and aligned with program objectives. To address this, a more structured and criteria-based selection process for appointing the most suitable CLs is currently under review and planned for implementation.

## **Budget**

![](media/image1.png){width="6.267716535433071in" height="3.388888888888889in"}

Funds are allocated to communities on a monthly basis. We only provide funding after the agreed progression has been achieved. We will provide monthly reporting to the community in the #ambassador channel.

The expenses can be tracked here through publicly available wallets broken down by local communities too. Community Leads will own their finances but Kimz, Raziah, 1 person elected by community leads form a finance committee to make sure funding is well spent.

[Payment report](https://docs.google.com/spreadsheets/d/1kOfbeXbwaZqNYP-77fkYWrmM_YbKnJVzW1co0BAYJmE/edit?usp=sharing)

Any funds that are left will be either carried over to the next ambassador program season or transferred back directly to the CCF.

## **Benchmark with other ambassador programs**

Ambassador Programs are popular in crypto and are funded differently depending on the size of the project and number of ambassadors. Ecosystems pushing for rapid global expansion now earmark $200 k-$500 k in stablecoins or liquid tokens per quarter. In 2025, Solana Superteam funded 500k a year. LunarStorm announced a 1M rewards sprint. The most important take away is that budgets scale with the community size and GTM goals.

## **Governance**

The Qubican program is run by some of Qubics biggest contributors. We call them Community Leads. Together they form the Qubican Quorum. Program decisions are made by Quorum. Local Community decisions are made by the Community Leads together with their workgroups. Anyone can apply to start a local community as long as the requirements are met. Below an overview of the current governance structure.

Ambassadors earn rewards by completing tasks for their community, such as creating content, newsletters, videos, organizing events, helping the ecosystem/marketing teams or other things that help grow the Qubic ecosystem. Program Managers and Community Leads are paid a monthly compensation, depending on the impact they had. Their impact is measured through the KPI's mentioned above.

Defimoma and Alber are not paid and act as liaisons for the Community Leads and ambassadors.

**Who's on the team?**

| Role | Name | Description |
| :--- | :--- | :--- |
| Marketing Liaison | Defimomma | Informs and trains ambassadors on marketing developments |
| Program Manager East | Kimz | Onboarding of new Communities, recruitment of CL, creates alignment between different communities . Focus on MENA and SEA |
| Ecosystem Liaison | Alber | Trains ambassadors on ecosystem developments, pitching, business development |
| Program Manager West / Community Lead France | Raziah | Onboarding of new Communities, recruitment of CL, creates alignment between different communities . Focus on MENA and SEA |
| Community Lead France | Seb | Coordinates local French Community and work groups |
| Community Lead Nomad | Popeye | Coordinates Nomad Community and work groups |
| Community Lead Nomads | vacant | Coordinates Nomad Community and work groups |
| Community Lead USA | Space caddet | Coordinates local US Community and work groups |
| Community Lead USA | Evadon | Coordinates local US Community and work groups |
| Community Lead Spain | Joobid | Coordinates local Spanish Community and work groups |
| Community Lead Spain | Arya | Coordinates local Spanish Community and work groups |
| Community Lead Nigeria | Emmanuel | Coordinates local Nigerian Community and work groups |
| Community Lead Nigeria | Xoddux | Coordinates local Nigerian Community and work groups |
| Community Lead China | Xingyuan | Coordinates local Chinese Community and work groups |
| Community Lead China | Vacant | Coordinates local Chinese Community and work groups |
| Community Lead Germany | Marty | Coordinates local German Community and work groups |
| Community Lead Germany | MDC | Coordinates local German Community and work groups |
| Community Lead Russia | Chinuk | Coordinates local Russian Community and work groups |
| Community Lead Russia | German | Coordinates local Russian Community and work groups |
| Community Lead Russia | Kavatak | Coordinates local Russian Community and work groups |
| Community Lead Vietnam | Arya | Coordinates local Vietnamese Community and work groups |
| Community Lead Vietnam | Tien Binfort | Coordinates local Vietnamese Community and work groups |
| Community Lead S.Korea | Parallel | Coordinates local S.Korean Community and work groups |
| Community Lead S.Korea | Vacant | Coordinates local S.Korean Community and work groups |
| Community lead Japan | Tendou | Coordinates local Japanese Community and work groups |
| Community lead Japan | Vacant | Coordinates local Japanese Community and work groups |
| Community lead Singapore | QiangZ | Coordinates local Singapore Community and work groups |
| Community lead Singapore | Vacant | Coordinates local Singapore Community and work groups |
